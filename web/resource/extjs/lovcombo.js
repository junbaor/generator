...
Ext.ux.form.LovCombo = Ext.extend(Ext.form.ComboBox, {
	...
	,
	initList : function() {
		Ext.ux.form.LovCombo.superclass.initList.call(this);
		if (this.checkAll && !this.checkEl) {
			this.checkEl = this.list.createChild({
				html : '<a href="javascript:void(0)" style="color:#000;TEXT-DECORATION: none">全选/反选</a>',
				cls : 'x-combo-list-all',
				style : 'text-align:right;color:#000;padding:5px;'
			});
			...
			this.assetHeight += this.checkEl.getHeight();
		}
	},
	...
	onRealBlur : function() {
		...
		// iterate through raw values and records and
		// check/uncheck items
		...
		if (v) {
			...
			if (this.valueField) {
				...
				this.store.each(function(r) {
		         ...
		         var checkedDisplay = this.getCheckedDisplay();
		         if(null != checkedValue && '' != checkedValue){
		        	 this.value = checkedValue;
					 this.setRawValue(checkedDisplay);
		         }else{
		        	 this.value = v;
					 this.setRawValue(v);
		         }
	...
	assertValue : function() {
		...
		if (!arr_rec && this.forceSelection) {
			...
			if (arr_rec && this.valueField) {
				// onSelect may have already set the value and
				// by doing so
				// set the display field properly. Let's not
				// wipe out the
				// valueField here by just sending the
				// displayField.
				if (this.value == val) {
					return;
				}

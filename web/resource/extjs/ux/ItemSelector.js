...
Ext.ux.form.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    iconLeftAll:'left-all.gif',
    iconRightAll:'right-all.gif',
    iconTop:"top2.gif",
    ...
    onRender: function(ct, position){
        ...
        this.iconLeftAll = this.imagePath + (this.iconLeftAll || 'left-all.gif');
        this.iconRightAll = this.imagePath + (this.iconRightAll || 'right-all.gif');
        ...
        el.createChild({tag: 'br'});
        this.addAllIcon=el.createChild({tag:'img', src:this.iconRightAll, style:{cursor:'pointer', margin:'2px'}});
        el.createChild({tag: 'br'});
        this.removeIcon = el.createChild({tag:'img', src:this.iconLeft, style:{cursor:'pointer', margin:'2px'}});
        el.createChild({tag: 'br'});
        this.removeAllIcon = el.createChild({tag:'img', src:this.iconLeftAll, style:{cursor:'pointer', margin:'2px'}});
        el.createChild({tag: 'br'});
        ...
        this.addAllIcon.on('click',this.fromAllTo,this);
        ...
        if (!this.drawLeftAllIcon || this.hideNavIcons) { this.addAllIcon.dom.style.display='none'; }
        if (!this.drawRightAllIcon || this.hideNavIcons) { this.removeAllIcon.dom.style.display='none'; }
    ...
    toTop : function() {
        ...
        this.fireEvent('selected',this,records);
    },
    ...
    fromAllTo:function(){
        var count = this.fromMultiselect.view.store.getCount(),selectionsArray=[],records=[];
        if (count > 0) {
    ...
    toFromAll:function(){
        var count = this.toMultiselect.view.store.getCount(),selectionsArray=[],records=[];
        if (count > 0) {
            for (var i=0; i<count; i++) {
